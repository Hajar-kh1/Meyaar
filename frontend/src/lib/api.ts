...
export async function interpretTeamCommands(instruction: string, context?: string): Promise<TeamCommandPlan> { return parseResponse(await fetch(`${API_BASE_URL}/team/commands/interpret`, { method: "POST", headers: { "Content-Type": "application/json", ...authHeaders() }, body: JSON.stringify({ instruction, context: context || null }) })); }
