...
import { checkBackendHealth, getAuthToken, getMe, logout, processVectorFile, updatePresence } from "@/lib/api";
    ...
    <div className="min-h-screen bg-background pb-20 text-slate-950 lg:pb-0">
      ...
      <div className="lg:pl-56 rtl:lg:pl-0 rtl:lg:pr-56">
        ...
        <main className="mx-auto w-full max-w-[1500px] p-3 sm:p-4 lg:p-5 xl:p-6">
          ...
          {result && view === "analysis" && !showBatchOverview && <div className="space-y-6">{batchUploads.length > 1 && <button type="button" onClick={() => setShowBatchOverview(true)} className="rounded-xl border border-blue-200 bg-white px-4 py-2 text-sm font-bold text-blue-700">← Back to uploaded files</button>}{filters}{vectorResult ? <><MapPanel result={displayedResult ?? vectorResult} selectedErrorId={selectedErrorId} /><ErrorTable result={displayedResult ?? vectorResult} selectedErrorId={selectedErrorId} onSelectError={selectError} /></> : <VisionPreview result={result as VisionAnalysisResponse} imageUrl={imageUrl} />}</div>}
