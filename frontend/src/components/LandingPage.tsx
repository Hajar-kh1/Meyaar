  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background text-[#071c33]">
      ...
      <section id="how-it-works" className="scroll-mt-24 bg-[#f7faf7] px-6 py-20 sm:px-12 lg:px-20">
        <div className="mx-auto max-w-6xl text-center"><p className="text-xs font-black uppercase tracking-[.22em] text-blue-600">{t("How the platform works")}</p><h2 className="mt-3 text-3xl font-black text-[#17332f]">{t("From data to a reliable decision")}</h2><div className="mt-10 grid gap-4 md:grid-cols-3">{[{ title: "Upload your data", body: "Choose geospatial files or map imagery." }, { title: "We validate quality", body: "Meyaar applies quality, geometry, and topology rules." }, { title: "Review results", body: "Inspect mapped issues and follow recommendations." }].map((step, index) => <article key={step.title} className="rounded-2xl border border-blue-100 bg-white p-7 text-start shadow-sm"><span className="flex size-10 items-center justify-center rounded-full bg-blue-600 text-sm font-black text-white">{index + 1}</span><h3 className="mt-5 text-xl font-bold text-[#17332f]">{t(step.title)}</h3><p className="mt-2 text-sm leading-7 text-slate-600">{t(step.body)}</p></article>)}</div></div>
      </section>

      <section id="about" className="scroll-mt-24 bg-white px-6 py-20 sm:px-12 lg:px-20">
        <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-[1fr_.8fr]"><div><p className="text-xs font-black uppercase tracking-[.22em] text-blue-600">{t("About Meyaar")}</p><h2 className="mt-3 text-3xl font-black text-[#17332f]">{t("A Saudi platform for geospatial data quality")}</h2><p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">{t("Meyaar helps data teams detect spatial errors, measure compliance, and understand results through interactive maps and AI-assisted recommendations.")}</p></div><div className="rounded-3xl border border-blue-100 bg-blue-50 p-8"><div className="grid grid-cols-2 gap-3 text-center"><div className="rounded-2xl bg-white p-5"><strong className="text-lg text-blue-700">{language === "ar" ? t("Saudi geospatial standards") : "GeoSA"}</strong><p className="mt-1 text-xs text-slate-500">{t("Aligned rules")}</p></div><div className="rounded-2xl bg-white p-5"><strong className="text-lg text-blue-700">{language === "ar" ? t("Artificial intelligence") : "AI"}</strong><p className="mt-1 text-xs text-slate-500">{t("Clear guidance")}</p></div></div></div></div>
      </section>
    </main>
  );
}
