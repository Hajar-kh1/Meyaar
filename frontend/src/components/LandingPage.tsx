  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background text-[#071c33]">
      ...
      <section id="about" className="scroll-mt-24 bg-white px-6 py-20 sm:px-12 lg:px-20">
        <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-[1fr_.8fr]"><div><p className="text-xs font-black uppercase tracking-[.22em] text-blue-600">{language === "ar" ? "عن معيار" : "About Meyaar"}</p><h2 className="mt-3 text-3xl font-black text-[#17332f]">{language === "ar" ? "منصة سعودية لجودة البيانات المكانية" : "A Saudi platform for geospatial data quality"}</h2><p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">{language === "ar" ? "يساعد معيار فرق البيانات على اكتشاف الأخطاء المكانية، قياس الالتزام، وفهم النتائج من خلال خرائط تفاعلية وتوصيات مدعومة بالذكاء الاصطناعي." : "Meyaar helps data teams detect spatial errors, measure compliance, and understand results through interactive maps and AI-assisted recommendations."}</p></div><div className="rounded-3xl border border-blue-100 bg-blue-50 p-8"><div className="grid grid-cols-2 gap-3 text-center"><div className="rounded-2xl bg-white p-5"><strong className="text-2xl text-blue-700">GeoSA</strong><p className="mt-1 text-xs text-slate-500">{language === "ar" ? "قواعد متوافقة" : "Aligned rules"}</p></div><div className="rounded-2xl bg-white p-5"><strong className="text-2xl text-blue-700">AI</strong><p className="mt-1 text-xs text-slate-500">{language === "ar" ? "توصيات مفهومة" : "Clear guidance"}</p></div></div></div></div>
      </section>
    </main>
  );
}
