...
export default function TeamManagementAssistant({ data, onClose, onComplete }: Props) {
  ...
  function browserSpeak(text: string) {
    ...
    if (speechLanguage === "ar-SA" && !voice) throw new Error("Arabic browser voice is unavailable.");
  ...
  async function speak(text: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.detail || "Server voice is unavailable.");
      }
      ...
      catch { setError(/[\u0600-\u06FF]/.test(text) ? "الصوت العربي يحتاج قبول شروط نموذج Groq Arabic Saudi من حساب Groq، ثم إعادة المحاولة." : (reason instanceof Error ? reason.message : "Audio could not be played on this device.")); }
    }
  }
...
function QuickActionIcon({ index }: { index: number }) {
  if (index === 0) return <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3.5 19c.5-4 2.3-6 5.5-6s5 2 5.5 6M18 7v6M15 10h6"/></svg>;
  if (index === 1) return <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8" cy="8" r="3"/><circle cx="17" cy="9" r="2.5"/><path d="M2.5 19c.5-4 2.3-6 5.5-6s5 2 5.5 6M14 14c3.8-.7 6.2 1 6.8 5"/></svg>;
  if (index === 2) return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 4h6v6M20 4l-7 7M10 20H4v-6M4 20l7-7"/><circle cx="8" cy="8" r="3"/><circle cx="16" cy="16" r="3"/></svg>;
  if (index === 3) return <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8" cy="9" r="3"/><circle cx="17" cy="9" r="3"/><path d="M2.5 20c.4-4 2.2-6 5.5-6s5.1 2 5.5 6M13 15c1-.7 2.2-1 4-1 3.3 0 5.1 2 5.5 6"/></svg>;
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20V10M10 20V4M16 20v-7M22 20H2"/><path d="m3 7 6-4 6 6 6-5"/></svg>;
}
