...
export default function VisionPreview({ result, imageUrl }: VisionPreviewProps) {
  ...
  const titleMissing = result.elements.some((element) => element.element === "title" && !element.present);
  const arabic = language === "ar";
  const copy = arabic ? { add: "أضيفي عنوانًا", placeholder: "اكتبي عنوان الخريطة", remove: "حذف العنوان", download: "تنزيل نسخة بعنوان", position: "موضع العنوان", left: "يسار", center: "وسط", right: "يمين", note: "هذه معاينة مؤقتة؛ الصورة الأصلية لن تتغير." } : { add: "Add a title", placeholder: "Enter map title", remove: "Remove title", download: "Download titled copy", position: "Title position", left: "Left", center: "Center", right: "Right", note: "This is a temporary preview; your original image is unchanged." };
  ...
  return (
    <section className="grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:grid-cols-2">
      <div className="flex min-h-80 items-center justify-center bg-slate-100 p-5">
        {imageUrl ? <div className="relative max-w-full"><Image src={imageUrl} alt={`Uploaded map ${result.filename}`} width={1200} height={800} unoptimized className="max-h-[520px] h-auto max-w-full rounded-xl object-contain shadow" />{title.trim() && <div className={`absolute top-4 max-w-[calc(100%-2rem)] rounded-md bg-white/90 px-3 py-1.5 text-center text-sm font-extrabold text-[#071c33] shadow-sm sm:text-base ${placement === "left" ? "left-4" : placement === "right" ? "right-4" : "left-1/2 -translate-x-1/2"}`}>{title}</div>}</div> : <p className="text-sm text-slate-500">Image preview unavailable.</p>}
      </div>
      <div className="p-6">
        ...
        {titleMissing && <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-bold text-blue-950">{copy.add}</h3>{title && <button type="button" onClick={() => setTitle("")} className="text-xs font-bold text-red-600 hover:text-red-700">{copy.remove}</button>}</div><p className="mt-1 text-xs text-blue-800">{copy.note}</p><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={copy.placeholder} className="mt-3 w-full rounded-xl border border-blue-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500" /><div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-xs font-bold text-blue-900">{copy.position}</span>{(["left", "center", "right"] as const).map((option) => <button key={option} type="button" onClick={() => setPlacement(option)} className={`rounded-lg px-2.5 py-1.5 text-xs font-bold ${placement === option ? "bg-blue-600 text-white" : "bg-white text-blue-700 ring-1 ring-blue-200"}`}>{copy[option]}</button>)}</div>{title.trim() && <button type="button" onClick={() => void downloadTitledCopy()} className="mt-3 rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-700">{copy.download}</button>}</div>}
